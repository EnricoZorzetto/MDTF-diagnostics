...
for case_name, case_dict in case_info.items():
    ds = xr.open_dataset(case_dict["SNOW_FILE"], use_cftime=True)
...
for k, v in case_info.items():
    # take the time mean
    arr = model_datasets[k][case_info[k]["snow_var"]]
...
# compute time average field of snow differences btw the two models
print("EZSNOW: case keys = {}".format(case_info.keys()))
snow_maps_diff =  snow_maps['ESM4-GLASS'] - snow_maps['ESM4-CM'] 
...
# add title
plt.title("Zonal Mean snow [kg/m2]")
...
# Part 3b: Plot the differences between the two models
# --------------------------------------------------------
fig = plt.figure(figsize=(12, 4))
ax = plt.subplot(1, 1, 1)
snow_maps_diff.plot(ax = ax)
plt.legend()
plt.title("Map of snow differences [kg/m2]")
plt.savefig(f"{work_dir}/model/PS/map_of_snow_diff.pdf", bbox_inches="tight", dpi=300)
# plt.savefig(f"{work_dir}/model/PS/map_of_snow_diff.eps")
