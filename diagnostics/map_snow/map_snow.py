...
input_path = os.environ["SNOW_FILE"]
...
#
snow_var_name = os.environ["snow_var"]
...
snow_data = model_dataset[snow_var_name]
model_mean_snow = snow_data.mean(dim = time_coord_name)
...
print("Computed time average of {snow_var} for {CASENAME}.".format(**os.environ))
...
#
out_path = "{WK_DIR}/model/netCDF/snow_means.nc".format(**os.environ)

# write out time averages as a netcdf file
model_mean_snow.to_netcdf(out_path)
...
title_string = "{CASENAME}: mean {snow_var} ({FIRSTYR}-{LASTYR})".format(**os.environ)
# Plot the model data:
plot_and_save_figure("model", title_string, model_mean_snow)
...
input_path = "{OBS_DATA}/example_snow_means.nc".format(**os.environ)

# command to load the netcdf file
# obs_dataset = xr.open_dataset(input_path)
# obs_mean_tas = obs_dataset['mean_snow']

# obs_dataset = xr.open_dataset(input_path)
# obs_mean_tas = obs_dataset['mean_snow']
obs_mean_snow = model_mean_snow 

# Plot the observational data:
title_string = "Observations: mean {snow_var}".format(**os.environ)
plot_and_save_figure("obs", title_string, obs_mean_snow)
